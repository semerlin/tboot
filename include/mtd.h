...
struct mtd_oob_ops
{
	...
	uint8_t *datbuf;  /* data数据缓冲区，为NULL时只有oob数据会被读写 */
	uint8_t *oobbuf;  /* oob数据缓冲区 */
};
...
struct mtd_info
{
	...
	int32_t (*read_oob) (struct mtd_info *mtd, loff_t from,
			 struct mtd_oob_ops *ops);
	int32_t (*write_oob) (struct mtd_info *mtd, loff_t to,
			 struct mtd_oob_ops *ops);
